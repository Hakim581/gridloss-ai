"""Interval balances using measured inputs and estimated technical losses."""
...
def interval_balances(simulation) -> tuple[pd.DataFrame, pd.DataFrame]:
    cfg = simulation.config["simulation"]
    hours = cfg["interval_minutes"] / 60
    meters = simulation.meters.copy()
    meters["slot"] = meters.timestamp.dt.hour * (60 // cfg["interval_minutes"]) + meters.timestamp.dt.minute // cfg["interval_minutes"]
    baseline_end = meters.timestamp.min() + pd.Timedelta(days=cfg["baseline_days"])
    reference = meters.loc[meters.timestamp < baseline_end].groupby(["customer_id", "slot"]).reported_kwh.median().rename("reference_kwh")
    ...
    aggregate = meters.groupby(["timestamp", "feeder_id"], as_index=False).agg(
        metered_kwh=("reported_kwh", "sum"), estimated_missing_kwh=("estimated_missing_kwh", "sum"),
    ...
    balance["expected_technical_loss_kwh"] = feeder_loss_kwh(
        ...
        balance.feeder_id.map(cfg["feeder_resistance_ohm"]).to_numpy(),
        hours, cfg["line_voltage_v"], cfg["power_factor"],
    )
    balance["unexplained_kwh"] = balance.input_kwh - balance.accounted_kwh - balance.expected_technical_loss_kwh

    by_time = simulation.feeders.groupby("timestamp", as_index=False).input_kwh.sum().rename(columns={"input_kwh": "feeder_inputs_kwh"})
    ...
    transformer["expected_technical_loss_kwh"] = transformer_loss_kwh(
        transformer.feeder_inputs_kwh.to_numpy(), hours, cfg["transformer_core_loss_kw"], cfg["transformer_copper_coefficient"]
    )
    transformer["unexplained_kwh"] = transformer.input_kwh - transformer.feeder_inputs_kwh - transformer.expected_technical_loss_kwh
    return balance, transformer


def daily_balances(interval: pd.DataFrame) -> pd.DataFrame:
    frame = interval.copy()
